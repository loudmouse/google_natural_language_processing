...
tokens.each do |token|
  if token.part_of_speech.tag.to_s.eql? 'NOUN'
    nouns << token
  end
  ...
  if token.part_of_speech.tag.to_s.eql? 'VERB'
    verbs << token
  end
  if token.part_of_speech.tag.to_s.eql? 'ADV'
    adverbs << token
  end
  if token.part_of_speech.tag.to_s.eql? 'ADJ'
    adjectives << token
  end
